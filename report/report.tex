...
\label{sec:structure}
In this section, we present our approach to learning the structure of influence graphs in collaborative sites. We first present an overview of the general problem, followed by our approach to learning structure, and finally validate our approach by learning structure graphs for three real-world datasets, \textit{reddit}, \textit{StackExchange}, and \textit{Something Awful}.
...
Given a graph structure, we are faced with a difficult inference problem: detecting cascades in networks of subsites. We leverage the exponential family techniques from \cite{wainwright} to learn a parameterized model. In contrast to NetInf \cite{netinf}, where the strength of connections between sites is used merely to visualize and explore the data, we learn a valid probability distribution over nodes that can predict future data. We validate our approach on the Meme Tracker \cite{memetracker} dataset. Due to the time constraints of the project, we were unable to gather sufficient data from reddit, SE, or SA; these sites are left as future work.


\subsection{Problem statement}
Given a graphical model, $\mathcal{G} = (\mathcal{V}, \mathcal{E})$, created as specified in Section \ref{algorithm}. 
Given nodes, $\mathcal{V}$, with overlapping connections, $\mathcal{C}$, via shared users, predict topic vector $v_{d,t+1}$ in document collection $d$ at time $t+1$ given the topic  topics $V$ in all documents at time $t$.
...
We model this domain as an unfolding Markov Chain, in which at each timestep, nodes may become infected. See the below graph, in which nodes $\{A,B,C\}$ represent the states of three sites at time $t$, and the nodes $\{A',B',C'\}$ represent the same nodes at time $t+1$.
